...
class DomainValidation
{
    ...
    public static function strCanNullOrMinLength(string $value, int $minLength = 5, string $exceptMessage = "")
    {
        if(!empty($value) && strlen($value) < $minLength) {
            throw new EntityValidationException($exceptMessage ?? "The value is null or must be greater than {$minLength} characters");
        }
    }

    public static function intRequired(string $value, int $min = 1, string $exceptMessage = "")
    {
        if($value < $min) {
            throw new EntityValidationException($exceptMessage ?? "The value is required and must be greater than {$min} characters");
        }
    }
}
